...
function reloadQuilkJson( cb ){
    try{
        quilkConf = global.quilkConf = require( pwd + '/quilk.json');
    } catch ( e ){
        if( !cliArgs.init ) {
            log.error('quilk.json not found or could not be read: Please either manually create a quilk.json file or run quilk init, see the readme for more.' );
            log.general( e );
            process.exit();
        }
    }
...
var waiting = false;
var haltBuild = false;
function build( modules_received, cb, no_final_stats ) {
          ...
          if (modules.length > 0) {
              if(!haltBuild){
                  ...
                  try {
                      ...
                      try{
                          ...
                          try{
                              ...
                              log.general('');
                              log.general('');
                              return logTime('Aborting quilk. Could not find the module in the default quilk list or your own custom list. Please review the errors above.')
                          }
                      }
                  }
                  ...
                  try{
                      ...
                      log.general( e );
                      return logTime('Aborting quilk. An error was found in the above module.')
                  }
              } else {
                  log.general( ' ');
...
function releaseCommandRunner( commands, cb ){
    if( commands.length > 0 ){
        ...
        if( Array.isArray( command )){
            ...
            log.loud( 'RELEASE COMMAND STARTING' );

            command_run( command.shift(), command, function(){
                releaseCommandRunner(commands, cb);
            } );
        } else if( typeof command === 'object' ){
            log.loud( 'RELEASE MODULE RUNNING: ' + command.module );
            ...
            exec( command, function( error, stdout, stderr ){
                if( error ){
                    ...
                    log.general( error );
                    log.general( stderr );
                    logTime( 'If the max buffer was exceeded, consider running this in array format.' );
                } else {
                    log.general( stdout );
                    return releaseCommandRunner(commands, cb);
                }
            ...
            log.error('ERROR');
            log.general('Relase object recieved in wrong format')
            return releaseCommandRunner(commands, cb);
        }
    } else {
        log.success( 'All release commands run.' );
        cb();
    }
}
    ...
    if( cliArgs.watch ){
        ...
        .on('all', function (event, path) {
            if (runOnEvents.indexOf(event) !== -1 && allAdded ) {
                ...
                // A change in the quilk.json file should trigger a fresh build of all modules, thus set it to false
                if( changeOn.indexOf('quilk.json') !== -1 ){
                    changeOn = false;
                }

                if( watch_ignore_func( changeOn ) ) {
                    if( waiting ){
                        log.general( 'Already waiting for a previous queue to finish.' );
                    } else {
                        log.general('Event "' + event + '" heard on path: ' + global.chokidarFileChangePath );
                        if( building ){
                            waiting = true;
...
if (cliArgs.release) {
    if( quilkConf.release_commands_or_modules[cliArgs.release].pre ){
        ...
        releaseCommandRunner(quilkConf.release_commands_or_modules[cliArgs.release].pre, function () {
            build( quilkConf.modules, function(){
                if( quilkConf.release_commands_or_modules[cliArgs.release].post ){
                    releaseCommandRunner(quilkConf.release_commands_or_modules[cliArgs.release].post, function(){
