...
function releaseCommandRunner( commands, cb ){
    if( commands.length > 0 ){
        ...
        switch( typeof command){
            case 'string': {
                log.loud( 'RELEASE COMMAND RUNNING: ' + command );
                exec( command, function( error, stdout, stderr ){
                    if( error ){
                        log.error( 'ERROR: Release script' );
                        console.log( error );
                        console.log( stderr );
                        log.error( 'ERROR: Release script' );
                        process.exit();
        ...
        log.success( 'All release commands run, now.' );
        cb();
    }
}
            ...
            if (runOnEvents.indexOf(event) !== -1 && allAdded ) {
                ...
                if( watch_ignore_func( changeOn ) ) {
                    if( waiting ){
                        ...
                        console.log('Event "' + event + '" heard on path: ' + global.chokidarFileChangePath );
                        if( building ){
                            waiting = true;
...
if (cliArgs.release) {
    ...
    console.log(quilkConf.release_commands_or_modules[cliArgs.release]);
