...
module.exports = function(){
    ...
    if( global.cliArgs.module ){
        ...
        try{
            ...
            try{
                ...
                log.general(clc.bold.red('ERROR finding module'));
                logTime( e );
                return logTime( e2 );
            }
        }
    ...
    function header( msg ){
        log.general( ' ' );
        log.general( clc.bold('>>> ') + clc.bold.underline(msg)  + clc.bold(' <<<') );
    }
    function header_end( msg ){
        ...
        log.general( clc.bold.underline(spacer_line)  );
    }
    ...
    for( var key in available_commands ){
        log.general( clc.bold( key ) );
        log.general( 'Description: ' + available_commands[key].msg );
        log.general( 'Example: ' + available_commands[key].eg );
        log.general( ' ' );
    }
    ...
    header( 'Available default modules for the quilk.json' );
    for( var key in default_modules ){
        log.general( clc.bold( key ) );
        log.general( default_modules[key].module_example );
        log.general( ' ' );
    }
    log.general( clc.bold( 'For module specific help just run: ' ) + 'quilk help module=<module_name>' );
    log.general( 'For quilk custom modules private or public, to use or create, please see the quilk readme file.' );
    log.general(' ');
    ...
    for( var key in developer_options ){
        log.general( clc.bold( key ) );
        log.general( developer_options[key] );
        log.general( ' ' );
    }
};