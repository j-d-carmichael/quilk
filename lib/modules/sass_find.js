    ...
    run: function( next ) {
        ...
        var ignoreFunc = function(file, stats) {
            ...
            for( var i=0;i<ignorePaths.length;++i ){
                console.log( 'ignored', ignorePaths[i] );
                console.log( 'not ignored', file );
        ...
        var include_last = function( paths, cb ){
            if( paths.length === 0 ){
                return cb();
            } else {
                var path = paths.pop();
                writeLineToIndexScss( '@import ".'+ path +'";', function(){
                    include_last( paths, cb );
                });
            }
        };
