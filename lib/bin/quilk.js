...
function build( cb ) {
    ...
    function runquilks(modules) {
        ...
        if (modules.length > 0) {
            ...
            console.log('  ');
            console.log('  ');
            console.log('Total time to run quilks: ' + (Number(time()) - Number(start)) + 'seconds');
            if(!cliArgs.quiet){
                desktopNotify( 'quilk finished', 'Total time: ' + (Number(time()) - Number(start)) + 'seconds');
            }
...
if( quilkConf['dont_watch'] ){
    file_endings_ignore_array.concat( quilkConf['dont_watch'] );
}
...
var watcher = function(){
    if( cliArgs.watch ){
        ...
        chokidar.watch(quilkConf["folders_to_watch"], {
            ...
            if (runOnEvents.indexOf(event) !== -1) {
                ...
                if( watch_ignore_func( GLOBAL.chokidarFileChangePath ) ) {
                    if( waiting ){
                        ...
                        console.log('Event "' + event + '" found in quilk.json so running now.');
                        if( building ){
                            waiting = true;
